       ...
       <div style="margin-top:50px;margin-right:50px;margin-left:50px;font-family: Laihu;">
        <el-table
         ...
         <div align="right" style="margin-top: 20px;">
            <el-button @click="handleNewEdit()" style="font-family: Laihu;">新增分类</el-button>
        </div>
        ...
        <el-dialog title="更改分类" :visible.sync="dialogForm2Visible" style="font-family: Laihu;" :close-on-click-modal=false>
                <el-form ref="updateform" :model="updateform" :rules="rules"  label-position="left"  >
                    <el-form-item label="分类名称"  prop="name" style="font-family: Laihu;">
                        <el-input type="text" v-model="updateform.name"  placeholder="请输入分类名称" ></el-input>
                    </el-form-item>
                </el-form>
        ...
        <el-dialog title="新增分类" :visible.sync="dialogFormVisible" style="font-family: Laihu;" :close-on-click-modal=false>
...
export default {
    ...
    methods: {
        loadlist(){
            var _this=this
            this.$axios.get("/types/?pageNum="+_this.pageNum)
            .then(res => {
                _this.loading=false
                console.log(res)
                if(res.status=== 200){
        ...
        handleDelete(index, row){
            ...
            this.$axios.get("/types/"+row.id+"/delete")
            .then(res => {
                console.log(res)
                if(res.status === 200){
                   this.$message({
        ...
        update(updateform){
            ...
            this.$refs[updateform].validate(valid=>{
                if(valid){
                    this.$axios.post("/types/"+this.updateform.id,{
